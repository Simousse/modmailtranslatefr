...
function initBaseMessageHandlers() {
  ...
  bot.on("messageUpdate", async (msg, oldMessage) => {
    ...
    if (threadMessage.isFromUser()) {
      const editMessage = utils.disableLinkPreviews(`**Edit du message:**\n\`Ancien:\` ${oldContent}\n\`Nouveau:\` ${newContent}`);
  ...
  bot.on("messageCreate", async msg => {
    ...
    if (mainGuilds.length === 1) {
        content = `Bot mentionné dans ${channel.mention} par ${userMentionStr}: "${msg.content}"\n\n<${messageLink}>`;
    } else {
        content = `Bot mentionné dans ${channel.mention} (${channel.guild.name}) par ${userMentionStr}: "${msg.content}"\n\n<${messageLink}>`;
    }
