...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function removeExpiredBlocks() {
    ...
    for (const userId of expiredBlocks) {
      ...
      logChannel.createMessage({
        content: `<@!${userId}> (id \`${userId}\`) n'est plus bloqué par le bot.`,
        allowedMentions: {
          users: [userId],
        },
      });
  ...
  const blockCmd = async (msg, args, thread) => {
    ...
    if (isBlocked) {
      channel.createMessage("Cet personne est déjà bloqué.");
      return;
    }
    ...
    if (expiresAt) {
      ...
      msg.channel.createMessage(`<@${userIdToBlock}> (id \`${userIdToBlock}\`) est désormais bloqué pour ${humanized}`);
      ...
      msg.channel.createMessage(`<@${userIdToBlock}> (id \`${userIdToBlock}\`) est désormais bloqué à vie.`);
  ...
  const unblockCmd = async (msg, args, thread) => {
    ...
    if (! isBlocked) {
      msg.channel.createMessage("Cet personne n'est pas bloqué.");
      return;
    }
    ...
    if (unblockAt) {
      ...
      msg.channel.createMessage(`<@${userIdToUnblock}> (id ${userIdToUnblock}) est désormais débloqué.`);
    ...
    if (blockStatus.isBlocked) {
      if (blockStatus.expiresAt) {
        msg.channel.createMessage({
          content: `<@!${userIdToCheck}> (id \`${userIdToCheck}\`) est bloqué jusqu'au ${blockStatus.expiresAt} (UTC)`,
          allowedMentions: { users: [userIdToCheck] },
        });
      } else {
        msg.channel.createMessage({
          content: `<@!${userIdToCheck}> (id \`${userIdToCheck}\`) est bloqué à vie.`,
          allowedMentions: { users: [userIdToCheck] },
        });
      }
    } else {
