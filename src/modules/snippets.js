  ...
  commands.addInboxServerCommand("snippet", "<trigger> [text$]", async (msg, args, thread) => {
    ...
    if (snippet) {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Le Snippet "${args.trigger}" existe déjà! Vous pouvez le modifier ou le supprimer avec ${config.prefix}edit_snippet et ${config.prefix}delete_snippet`);
      } else {
        // If the snippet exists and we're NOT trying to create a new one, show info about the existing snippet
        utils.postSystemMessageWithFallback(msg.channel, thread, `\`${config.snippetPrefix}${args.trigger}\` répondra automatiquement: \`\`\`\n${utils.disableCodeBlocks(snippet.body)}\`\`\``);
      }
    } else {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Le Snippet "${args.trigger}" à été créé!`);
      } else {
        // If the snippet doesn't exist and the user isn't trying to create it, inform them how to create it
        utils.postSystemMessageWithFallback(msg.channel, thread, `Le Snippet "${args.trigger}" n'existe pas! Vous pouvez le crée avec \`${config.prefix}snippet ${args.trigger} text\``);
      }
    }
  }, {
    aliases: ["s"]
