...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand("move", "<category:string$>", async (msg, args, thread) => {
    ...
    if (containsRankings[0][1] === 0) {
      thread.postSystemMessage("Cette catégorie n'a pas été trouvé.");
      return;
    }
    ...
    try {
      ...
      thread.postSystemMessage(`Impossible de move le ticket: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`Le ticket à été move dans ${targetCategory.name.toUpperCase()}, mais impossible de sync les permissions: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`Le ticket à été move dans ${targetCategory.name.toUpperCase()}`);
  });
};
