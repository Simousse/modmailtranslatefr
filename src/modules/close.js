...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      await sendCloseNotification(thread, `Le ticket #${thread.thread_number} avec ${thread.user_name} (${thread.user_id}) à été fermé avec un délai par ${thread.scheduled_close_name}`);
    }
  }
  ...
  commands.addGlobalCommand("close", "[opts...]", async (msg, args) => {
    ...
    if (msg.channel instanceof Eris.PrivateChannel) {
      ...
      if (args.cancel || opts.includes("cancel") || opts.includes("c")) {
        ...
        if (thread.scheduled_close_at) {
          ...
          thread.postSystemMessage("Fermeture du ticket annulée.");
        }
      ...
      if (delayStringArg) {
        ...
        if (delay === 0 || delay === null) {
          thread.postSystemMessage("Délai invalide, respectez le format: \"1h30m\"");
          return;
        }
        ...
        let response;
        if (silentClose) {
          response = `Le ticket sera fermé en silence dans ${utils.humanizeDelay(delay)}. Utiliser \`${config.prefix}close cancel\` pour annuler.`;
        } else {
          response = `Le ticket sera fermé dans ${utils.humanizeDelay(delay)}. Utiliser \`${config.prefix}close cancel\` pour annuler.`;
        }
    ...
    await sendCloseNotification(thread, `Le ticket #${thread.thread_number} avec ${thread.user_name} (${thread.user_id}) à été fermé par ${closedBy}`);
  }, {
    options: [
  ...
  bot.on("channelDelete", async (channel) => {
    ...
    console.log(`[INFO] Fermeture automatique du ticket avec ${thread.user_name} car le channel à été supprimé.`);
    ...
    await sendCloseNotification(thread, `Fermeture automatique du ticket #${thread.thread_number} avec ${thread.user_name} (${thread.user_id}) car le channel à été supprimé.`);
  });
};
