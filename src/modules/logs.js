...
module.exports = ({ bot, knex, config, commands, hooks }) => {
  ...
  const logsCmd = async (msg, args, thread) => {
    ...
    const threadLines = await Promise.all(userThreads.map(async userThread => {
      ...
      const formattedLogUrl = logUrl
        ...
        : `Voir les logs avec \`${config.prefix}log ${userThread.thread_number}\``
      const formattedDate = moment.utc(userThread.created_at).format("MMM Do [à] HH:mm [UTC]");
      return `\`#${userThread.thread_number}\` \`${formattedDate}\`: ${formattedLogUrl}`;
    }));

    let message = isPaginated
      ? `**Logs pour <@${userId}>** (Page **${page}/${maxPage}**, affichage des logs de **${start + 1} à ${end}** sur un total de** ${totalUserThreads}** tickets):`
      : `**Logs pour <@${userId}>:**`;

    message += `\n${threadLines.join("\n")}`;

    if (isPaginated) {
      message += "\nPour voir la suite, faites !logs Numéro de la page";
    }
  ...
  const logCmd = async (msg, args, _thread) => {
    ...
    if (logUrl) {
      channel.createMessage(`Cliquez sur le lien pour voir l'historique du ticket #${thread.thread_number}:\n<${addOptQueryStringToUrl(logUrl, args)}>`);
      return;
    }

    const logFile = await getLogFile(thread);
    if (logFile) {
      channel.createMessage(`Téléchargez le fichier pour voir l'historique du ticket #${thread.thread_number}:`, logFile);
      return;
    }
    ...
    channel.createMessage("Les logs de ce ticket ne sont pas disponiblent");
  };
