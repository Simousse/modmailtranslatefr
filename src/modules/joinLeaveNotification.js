...
module.exports = ({ bot }) => {
    ...
    bot.on("guildMemberAdd", async (guild, member) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***L'utilisateur à rejoint le serveur ${guild.name}.***`
        );
      }
    });
  }
      ...
      setTimeout(async () => {
        ...
        if (thread != null) {
          if (leaveIgnoreIDs.includes(member.id)) {
            ...
            await thread.postSystemMessage(
              `***L'utilisateur à quitté le serveur ${guild.name}.***`
            );
          }
        }
      }, 2 * 1000);
  ...
  if (config.notifyOnMainServerLeave) {
    bot.on("guildBanAdd", async (guild, user) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***L'utilisateur à été ban du serveur ${guild.name}.***`
        );
  ...
  if (config.notifyOnMainServerJoin) {
    bot.on("guildBanRemove", async (guild, user) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***L'utilisateur à été déban du serveur ${guild.name}.***`
        );
      }
    });
  }
