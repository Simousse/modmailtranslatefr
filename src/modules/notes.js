...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function userNotesCmd(msg, userId) {
    ...
    if (! userNotes.length) {
      msg.channel.createMessage({
        content: `Il n'y a pas de note pour <@!${userId}>`,
        allowedMentions: {},
      });
      return;
    }
    ...
    for (const userNote of userNotes) {
      ...
      const content = [
        `Note mise par <@!${userNote.author_id}> le <t:${timestamp}:f>:`,
        `${START_CODEBLOCK}${escapeMarkdown(userNote.body)}${END_CODEBLOCK}`,
        `*Vous pouvez la supprimer avec \`${config.prefix}delete_note ${userNote.id}\`*\n`,
      ].join("\n");
  ...
  async function deleteUserNoteCmd(msg, noteId) {
    ...
    if (! note) {
      postError(msg.channel, "Note non trouvé!");
      return;
    }

    await deleteNote(noteId);
    await msg.channel.createMessage(`Note supprimé pour <@!${note.user_id}>:\n${START_CODEBLOCK}${escapeMarkdown(note.body)}${END_CODEBLOCK}`);
  }
