...
class Thread {
  ...
  async _sendDMToUser(content, file = null) {
    ...
    if (! dmChannel) {
      throw new Error("Impossible d'ouvrir un DM avec cette personne.");
    }
  ...
  async replyToUser(moderator, text, replyAttachments = [], isAnonymous = false, messageReference = null) {
    ...
    if (! utils.messageContentIsWithinMaxLength(dmContent) || ! utils.messageContentIsWithinMaxLength(inboxContent)) {
      ...
      await this.postSystemMessage("La réponse est trop longue! Elle doit contenir moins de 2000 caractères");
      return false;
    }
    ...
    try {
      ...
      await this._deleteThreadMessage(threadMessage.id);
      await this.postSystemMessage(`Erreur quant à la réponse : ${e.message}`);
      return false;
    }
    ...
    if (this.scheduled_close_at) {
      ...
      await this.postSystemMessage("La fermeture programée du ticket à été annulée dû à une nouvelle réponse.");
    }
  ...
  async receiveUserReply(msg, skipAlert = false) {
    ...
    if (this.scheduled_close_at) {
      await this.cancelScheduledClose();
      await this.postSystemMessage(`<@!${this.scheduled_close_id}> La fermeture du ticket avec un délai à été annulée dû à une nouvelle réponse.`, {
        allowedMentions: {
  ...
  async close(suppressSystemMessage = false, silent = false) {
    if (! suppressSystemMessage) {
      console.log(`Fermeture du ticket ${this.id}`);

      if (silent) {
        await this.postSystemMessage("Fermeture du ticket en silence...");
      } else {
  ...
  async editStaffReply(moderator, threadMessage, newText, opts = {}) {
    ...
    if (! utils.messageContentIsWithinMaxLength(formattedDM) || ! utils.messageContentIsWithinMaxLength(formattedThreadMessage)) {
      await this.postSystemMessage("La réponse éditée est trop longue! Elle doit contenir moins de 2000 caractères");
      return false;
    }
