...
async function createNewThreadForUser(user, opts = {}) {
  return _addToThreadCreationQueue(async () => {
    ...
    infoHeaderItems.push(`Compte Discord crée il y a **${accountAge}**`);
    ...
    for (const [guildId, guildData] of userGuildData.entries()) {
      ...
      const headerItems = [
        `Nom sur ce discord : **${utils.escapeMarkdown(nickname)}**`,
        `à rejoint le serveur il y a **${joinDate}**`,
        `\n\u200bModmail by Dragory translate by Simon Warner#2815`
      ];
      ...
      if (guildData.member.voiceState.channelID) {
        ...
        if (voiceChannel) {
          headerItems.push(`EST EN VOCAL **${utils.escapeMarkdown(voiceChannel.name)}**`);
        }
      }
    ...
    if (userLogCount > 0) {
      infoHeader += `\n\nCette personne à **${userLogCount}** précédent(s) ticket(s). Utiliser \`${config.prefix}logs\` pour le(s) voir.`;
    }

    const userNotes = await findNotesByUserId(user.id);
    if (userNotes.length) {
      infoHeader += `\n\nCette personne à **${userNotes.length}** notes. Utiliser \`${config.prefix}notes\` pour le(s) voir.`;
    }
    ...
    if (config.updateNotifications) {
      ...
      if (availableUpdate) {
        await newThread.postNonLogMessage(`📣 Nouvelle verison du bot disponible (${availableUpdate})`);
      }
    }
